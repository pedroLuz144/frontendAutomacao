...
const Crud = () => {
    let usuarioVazio: Automacao.Usuario = {
        id: 0,
        nome: '',
        login: '',
        senha: '',
    ...
    const [usuarios, setUsuarios] = useState<Automacao.Usuario[]>([]);
    const [usuarioDialog, setUsuarioDialog] = useState(false);
    const [deleteUsuarioDialog, setDeleteUsuarioDialog] = useState(false);
    const [deleteUsuariosDialog, setDeleteUsuariosDialog] = useState(false);
    const [usuario, setUsuario] = useState<Automacao.Usuario>(usuarioVazio);
    ...
    const editUsuario = (usuario: Automacao.Usuario) => {
        setUsuario({ ...usuario });
        setUsuarioDialog(true);
    };

    const confirmDeleteUsuario = (usuario: Automacao.Usuario) => {
        setUsuario(usuario);
        ...
        usuarioService.excluir(usuario.id)
            .then((response) => {
                ...
                toast.current?.show({
                    severity: 'error',
                    summary: 'Erro!',
                    detail: 'Erro ao deletado o usuário!',
                    life: 3000
                });
            });
    };
    ...
    const idBodyTemplate = (rowData: Automacao.Usuario) => {
        return (
            <>
    ...
    const nomeBodyTemplate = (rowData: Automacao.Usuario) => {
        return (
            <>
    ...
    const loginBodyTemplate = (rowData: Automacao.Usuario) => {
        return (
            <>
    ...
    const emailBodyTemplate = (rowData: Automacao.Usuario) => {
        return (
            <>
    ...
    const actionBodyTemplate = (rowData: Automacao.Usuario) => {
        return (
            <>
                ...
                <div className="card">
                    ...
                    <Dialog visible={deleteUsuarioDialog} style={{ width: '450px' }} header="Confirmar" modal footer={deleteUsuarioDialogFooter} onHide={hideDeleteUsuarioDialog}>
                        <div className="flex align-items-center justify-content-center">
                            ...
                            {usuario && (
                                <span>
                                    Você tem certeza que quer excluir o usuario <b>{usuario.nome}</b>?
                                </span>
                            )}
                        </div>
                    </Dialog>
                    ...
                    <Dialog visible={deleteUsuariosDialog} style={{ width: '450px' }} header="Confirmar" modal footer={deleteUsuariosDialogFooter} onHide={hideDeleteUsuariosDialog}>
                        <div className="flex align-items-center justify-content-center">
                            <i className="pi pi-exclamation-triangle mr-3" style={{ fontSize: '2rem' }} />
                            {usuario && <span>Você tem certeza que quer excluir os usuarios selecionados?</span>}
                        </div>
                    </Dialog>
                </div>
            </div>
